...
`public/tamil_dictionary.txt` remains the Scrabble validation dictionary and can include generated playable inflected forms. Sandhi-linker surfaces are excluded from this dictionary because they are orthographic prose variants, not independent playable words. `static-word-list/lemma_dictionary.txt` is maintained separately as a source headword/lemma inventory for tokenizer use and excludes generated inflections.
